...
/*   Updated: 2018/10/12 19:54:16 by dlaurent         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	sh_command_expansion_len(t_shell *sh, char *str, int i)
{
	...
	if (str[i] == '?')
		return (str_size_base(10, sh->prompt.last_exec_succeed));
	...
	while (str[i])
	{
		if ((delim && str[i] == delim)
		|| (!delim && !ft_isalnum(str[i])))
			return (len);
		len++;
		i++;
	}
...
void		sh_command_expand_dollars(t_shell *sh, t_env *env, char *str)
{
	...
	while (str[++i])
		...
		&& sh_command_expansion_criteria(str[i + 1]))
		{
			...
			: sh_command_expansion_val(sh, env, ft_strsub(str, i + 1, len));
