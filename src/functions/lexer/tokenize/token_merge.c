...
/*   Updated: 2018/10/28 14:29:59 by rpinoit          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char    lexer_token_merge(t_lexer *lexer, size_t i)
{
    ...
    lexer->tokens[i].id = ft_strjoinf(lexer->tokens[i].id, lexer->tokens[i + 1].id, 3);
    lexer->tokens[i].type = (lexer->tokens[i].type == TOKEN_MERGE)
        ? lexer->tokens[i + 1].type
        : lexer->tokens[i].type;
    if ((i + 2) < lexer->size)
        ft_memmove(&lexer->tokens[i + 1], &lexer->tokens[i + 2], sizeof(t_token) * (lexer->size - i + 2));
    lexer->size -= 1;
    return (STATUS_OK);
}