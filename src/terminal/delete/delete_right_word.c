...
static void	sh_delete_words(t_shell *sh, int rel_start, int rel_stop)
{
	...
	i = 0;
	while (sh->buffer.content[rel_stop + i])
	{
		sh->buffer.content[rel_start] =
		sh->buffer.content[rel_stop + i];
		rel_start++;
		i++;
	}
	while (sh->buffer.content[rel_start])
	{
		sh->buffer.content[rel_start] = '\0';
		rel_start++;
	}
}
